...
O frontend usa `input type="datetime-local"` que retorna o formato: `"YYYY-MM-DDTHH:mm"` (ex: `"2025-11-20T14:30"`)

**Conversão no Frontend**: O frontend adiciona `:00` ao final antes de enviar:
- Input retorna: `"2025-11-20T14:30"`
- Frontend envia: `"2025-11-20T14:30:00"`
...
#### Sem `resolvido_em` (campo não enviado ou `null`):
```json
{
  "title": "Título do chamado",
  "nome_cliente": "Nome do Cliente",
...
**Observação**: O campo é completamente opcional. Se não for enviado, o sistema deve calcular automaticamente o tempo de resolução pela diferença entre `created_at` e `updated_at` quando o status for "resolvido" ou "finalizado".

---

### 2. Atualizar Ticket (PUT /api/tickets/{id})

#### Definir `resolvido_em`:
```json
{
  "status": "resolvido",
  "resolvido_em": "2025-11-20T16:45:00"
}
```

#### Remover `resolvido_em` (enviar `null`):
```json
{
  "resolvido_em": null
}
...
#### Atualizar sem alterar `resolvido_em`:
```json
{
  "status": "pendente",
  "priority": "alta"
  // resolvido_em não enviado - mantém valor atual
}
```
...
A resposta deve incluir o campo `resolvido_em`:
...
```json
{
  ...
  "status": "resolvido",
  "priority": "alta",
  "resolvido_em": "2025-11-20T14:30:00.000000Z",  // Se preenchido
...
**Se não estiver preenchido**: `resolvido_em` deve ser `null` ou não estar presente na resposta.
